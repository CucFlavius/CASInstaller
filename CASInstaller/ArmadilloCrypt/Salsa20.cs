...
public sealed class Salsa20 : SymmetricAlgorithm
{
    ...
    private sealed class Salsa20CryptoTransform : ICryptoTransform
    {
        ...
        {
            Debug.Assert(key != null && key.Length is 16 or 32, "key.Length == 16 || key.Length == 32",
                "Invalid key size.");
            Debug.Assert(iv != null && iv.Length == 8, "iv.Length == 8", "Invalid IV size.");
            Debug.Assert(rounds is 8 or 12 or 20, "rounds == 8 || rounds == 12 || rounds == 20",
        ...
        private void Initialize(byte[]? key, byte[]? iv)
        {
            ...
            var constants = key != null && key.Length == 32 ? c_sigma : c_tau;
            if (key != null)
            {
