...
public struct ArchiveIndex
{
    ...
    private ArchiveIndex(byte[] data)
    {
        if (data.Length == 0)
            return;
        ...
        while (recordsRead != numElements)
        {
            ...
            for (var blockIndex = 0; blockIndex < recordsPerBlock && recordsRead < numElements; blockIndex++, recordsRead++)
            {
                ...
                var entry = new IndexEntry(br, sizeBytes, offsetBytes);
    ...
    public struct IndexEntry
    {
        ...
        public IndexEntry(BinaryReader br, byte sizeBytes, byte offsetBytes)
        {
            if (sizeBytes == 4)
            {
    ...
    public static async Task<ArchiveIndex[]> GetDataIndexes(Hash[] keys, CDN cdn, string pathType, string data_dir)
    {
        var indexes = new ArchiveIndex[keys.Length];
        var tasks = new Task<ArchiveIndex>[keys.Length];
    ...
    public static async Task<ArchiveIndex> GetDataIndex(CDN cdn, Hash key, string pathType, string data_dir)
    {
        ...
        {
            return new ArchiveIndex(await File.ReadAllBytesAsync(savePath));
        }
        else
        {
